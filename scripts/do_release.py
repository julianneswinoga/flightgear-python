...
def apply_actions(actions: List[named_action]):
    ...
    yn = get_user_input(
        f"Would you like to apply the following actions:{actions_str}\r\n[y/n]",
        check_yn,
        conv_yn,
    )
    if yn == "y":
...
def get_user_input(
    prompt: str, check_fn: Optional[Callable] = None, conv_fn: Optional[Callable] = None
):
    while True:
...
def main():
    ...
    if git_info["latest_tag"] is None:
        # First release!
        yn = get_user_input(
            "No tags yet, would you like to create 1.0.0? [y/n]", check_yn, conv_yn
        )
        if yn == "n":
            clean_exit(0)
        ...
        is_increment = lambda x: version.parse(x) > version.parse(
            str(git_info["latest_tag"])
        )
        poetry_version_rules = ("patch", "minor", "major")
        check_semver_or_rule = (
            lambda x: (is_semver(x) and is_increment(x)) or x in poetry_version_rules
        )
        poetry_bump_str = get_user_input(
            f"Latest tag is {git_info['latest_tag']}, "
    ...
    git_create_tag_action_1 = named_action(
        ...
        lambda: git_info["repo"].create_tag(
            poetry_version_str, message=f'Automatic tag "{poetry_version_str}"'
        ),
    )
    ...
    old_auto_tag = next(
        (t.commit for t in git_info["repo"].tags if t.name == poetry_version_str), None
    )
