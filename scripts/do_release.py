...
def main():
    ...
    with sh.pushd(git_info['root']):
        ...
        git_add_pyproj_action = named_action(
            f'Add ./pyproject.toml changes to git',
            lambda: git_info['repo'].git.add('./pyproject.toml')
        )
    git_amend_action = named_action(
        f'Amend {real_changelog} changes to latest commit',
        lambda: git_info['repo'].git.commit('--amend', '--no-edit')
    )
    actions.append(git_add_changelog_action)
    actions.append(git_add_pyproj_action)
