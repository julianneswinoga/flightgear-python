...
def main():
    ...
    if git_info['latest_tag'] is None:
        # First release!
        yn = get_user_input('No tags yet, would you like to create 1.0.0? [y/n]', check_yn, conv_yn)
        if yn == 'n':
            clean_exit(0)
    ...
    git_empty_commit_action = named_action(
        'create empty commit', lambda: git_info['index'].commit(f'Autogenerated release {poetry_bump_str}')
    )
    poetry_set_version_action = named_action(
        f'Set poetry version to {poetry_bump_str}', lambda: sh.poetry.version('-q', poetry_bump_str)
    )
    ...
    with sh.pushd(git_info['root']):
        ...
        git_add_changelog_action = named_action(
            f'Add {real_changelog} changes to git', lambda: git_info['repo'].git.add(real_changelog)
        )
    ...
    git_delete_tag_action = named_action(
        f'Delete tag {poetry_version_str} @ {old_auto_tag}', lambda: git_info['repo'].delete_tag(poetry_version_str)
    )
