...
def represent_object(
    o: Any, ident_level: int = 0, context_str: Optional[str] = None
) -> Tuple[str, Optional[str]]:
    if isinstance(o, Struct):
        # Print struct members nicely, just a 'wrapper' around dict
        # recurse
        struct_dict = {s.name: s for s in o.subcons}
        val_str, context_str = represent_object(
            struct_dict, ident_level + 1, context_str
        )
        pp_str = "\n" + val_str
        ...
        pp_str = o.fmtstr
        context_str = (
            "See Python's struct format characters for understanding type sizes"
        )
        ...
        if isinstance(o, Array):
            ...
            # recurse
            val_str, context_str = represent_object(
                o.ksymapping, ident_level + 1, context_str
            )
            special_str = f"\n{val_str}\n"
...
class PrintValueDirective(Directive):
    ...
    def run(self):
        ...
        return [addnodes.desc_name(text=text_str), addnodes.desc_content("", literal)]
