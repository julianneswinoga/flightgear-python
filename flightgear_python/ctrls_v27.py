...
ctrls_struct = {
    ...
    "oil_press_status": Array(
        FG_MAX_ENGINES, Enum(Int32ub, normal=0, low=1, full_fail=2)
    ),
    ...
    "xfer_pump": Array(
        5, Int32ub
    ),  # specifies transfer from array value tank to tank specified by int value
    ...
    "freeze": BitStruct(  # Default is big-endian
        other=BitsInteger(
            (Int32ub.sizeof() * 8) - 3
        ),  # Rest of uint32 minus the 3 flags
        fuel=Bit,
        position=Bit,
        master=Bit,
    ),
