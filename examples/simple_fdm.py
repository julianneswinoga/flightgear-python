#!/usr/bin/python3
"""
Simple Flight Dynamics Model (FDM) example that makes the altitude
increase and the plane roll in the air.
...
(you probably also want `--fdm=null` and `--max-fps=30` to stop the
simulation fighting with these external commands)
"""
import time
...
if __name__ == "__main__":  # NOTE: This is REQUIRED on Windows!
    ...
    while True:
        phi_rad_parent += 0.1
        # could also do `fdm_conn.event_pipe.parent_send` so you just
        # need to pass around `fdm_conn`.
