...
"""
TODO: DOCS
Simple example where we connect to both the FDM and Controls interface
to create closed loop control. We're using a simple Proportional ('P')
controller that reads the plane roll angle and controls the ailerons.
...
from flightgear_python.fg_if import CtrlsConnection, FDMConnection
...
def ctrls_callback(ctrls_data, event_pipe):
    ...
    if event_pipe.child_poll():
        ...
        # TODO: FG sometimes ignores "once" updates? i.e. if we set
        # `ctrls_data.aileron` the next callback will still have the old
        # value of `ctrls_data.aileron`, not the one that we set. To fix
        # this we can just keep our own state of what the value should
        # be and set it every time. I still need to figure out a clean
        # way to fix this on the backend.
...
if __name__ == "__main__":  # NOTE: This is REQUIRED on Windows!
    ...
    while True:
        if (
            fdm_event_pipe.parent_poll()
        ):  # Only update controller when we get FDM updates
            ...
            # Calculate the aileron request (technically should clamp to
            # -1..1 but it doesn't really matter)
