#!/usr/bin/python3
"""
Simple GUI example that prints the latitude and longitude of the
aircraft.

BEfore running the script, start FlightGear with
`--native-gui=socket,out,30,localhost,5504,udp --native-gui=socket,in,30,localhost,5505,udp`
"""
import math
import time
...
if __name__ == "__main__":  # NOTE: This is REQUIRED on Windows!
    ...
    # Note: I couldn't get FlightGear to do anything with the returned
    # data on this interface. I think it's just ignoring everything.
    # Technically you can send data back though.
    # gui_conn.connect_tx('localhost', 5505)
    gui_conn.start()  # Start the GUI RX loop

    while True:
        # could also do `gui_conn.event_pipe.parent_recv` so you just
        # need to pass around `gui_conn`.
